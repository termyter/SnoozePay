...
/// copy has no single screen: the same strings surface on the lock screen, in
/// the Dynamic Island and in a banner, all rendered by the system from one
/// `AlarmPresentation`. The shared area is that presentation, so it gets the
/// domain. The per-key `comment` in the catalogue records why each one is not
/// merged with the similarly-worded key next door.
enum AlarmKitCopy {

    // MARK: - Keys

    /// «Выключить будильник» — the words `StopAlarmIntent.title` renders.
    /// Read by the test that pins that literal, not yet by the intent itself.
    static let stopKey = "alarm_kit.action.stop"

    /// «Поспать ещё» — the words `SnoozeAlarmIntent.title` renders, under the
    /// same pinned-but-not-yet-read arrangement as ``stopKey``.
    static let snoozeKey = "alarm_kit.action.snooze"

    /// «Поспать ещё (−50 ₽)» — the alert's secondary button, which names the
    /// price. `%@` is a sum already formatted by ``MoneyFormatter``. This one
    /// is read at runtime.
