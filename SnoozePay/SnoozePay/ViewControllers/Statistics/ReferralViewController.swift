...
final class ReferralViewController: UIViewController {
    ...
    private let applyButton = SPButton(
        title: Localized.text("referral.button.apply"),
        variant: .quiet,
        size: .sm
    )
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = Localized.text("referral.title")
    ...
    private func makeHeroCard() -> UIView {
        ReferralHeroCardView(
            caps: Localized.text("referral.hero.caps"),
            headline: Localized.format("referral.hero.headline", MoneyFormatter.string(200)),
            body: Localized.text("referral.hero.body")
        )
    }
    ...
    private func makePersonalCodeSection() -> UIView {
        let caps = makeSectionCaps(Localized.text("referral.section.personal_code"))
        ...
        let copyButton = SPButton(title: Localized.text("referral.button.copy"), variant: .money, size: .sm)
    ...
    private func makeFriendCodeSection() -> UIView {
        let caps = makeSectionCaps(Localized.text("referral.section.friend_code"))
        let card = SPCard(tone: .surface, padding: AppSpacing.sp3)

        friendCodeField.placeholder = Localized.text("referral.field.friend_code_placeholder")
        ...
        let hint = UILabel()
        hint.text = Localized.text("referral.friend_code.hint")
    ...
    private func makeFriendsSection() -> UIView {
        let caps = makeSectionCaps(Localized.text("referral.section.friends"))
        ...
        let rowMasha = makeFriendRow(
            FriendRowSpec(
                initial: Localized.text("referral.friend_1.initial"),
                badgeColor: nil,
                badgeGradient: SPSupport.moneyGradientColors,
                badgeTextColor: AppColors.fgOnMoney,
                title: Localized.text("referral.friend_1.name"),
                subtitle: Localized.text("referral.friend_1.status"),
        ...
        let rowDima = makeFriendRow(
            FriendRowSpec(
                initial: Localized.text("referral.friend_2.initial"),
                ...
                title: Localized.text("referral.friend_2.name"),
                subtitle: Localized.text("referral.friend_2.status"),
                trailing: makeMetaLabel(text: Localized.text("referral.friend_2.trailing"), color: AppColors.warn400),
                divider: true
            )
        )
        let rowAnya = makeFriendRow(
            FriendRowSpec(
                initial: Localized.text("referral.friend_3.initial"),
                badgeColor: AppColors.whiteOverlay08,
                badgeGradient: nil,
                badgeTextColor: AppColors.fg3,
                title: Localized.text("referral.friend_3.name"),
                subtitle: Localized.text("referral.friend_3.status"),
                trailing: makeMetaLabel(text: Localized.text("referral.friend_3.trailing"), color: AppColors.fg3),
                divider: false
            )
        )
    ...
    private func makeShareButton() -> UIView {
        let button = SPButton(
            title: Localized.text("referral.button.share"),
            variant: .money,
            size: .lg,
            fullWidth: true
        )
    ...
    @objc private func applyFriendCodeTapped() {
        ...
        let alert = UIAlertController(
            title: Localized.text("referral.applied.title"),
    ...
    @objc private func shareTapped() {
        let text = Localized.format(
            "referral.share.message",
            Self.personalCode,
            MoneyFormatter.string(200)
        )
